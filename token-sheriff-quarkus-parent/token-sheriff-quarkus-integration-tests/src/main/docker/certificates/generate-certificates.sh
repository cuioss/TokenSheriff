...
CERT_DIR="${SCRIPT_DIR}"
CERT_DNAME="CN=localhost, OU=Integration Testing, O=Token-Sheriff, L=Berlin, ST=Berlin, C=DE"
CERT_VALIDITY=730
...
keytool -genkeypair \
  ...
  -dname "${CERT_DNAME}" \
  -ext san=dns:localhost,dns:keycloak,dns:dex,dns:zitadel,ip:127.0.0.1,ip:0.0.0.0 \
...
echo "SAN: dns:localhost,dns:keycloak,dns:dex,dns:zitadel,ip:127.0.0.1,ip:0.0.0.0"
echo ""
