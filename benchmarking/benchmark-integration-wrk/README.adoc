...
* **Docker Image**: `wrk:local` built from `Dockerfile.wrk` (Alpine-based, multi-arch)
* **Execution**: Via `docker compose run --rm wrk` using the `wrk` service defined in `docker-compose.yml`
...
* **Java Post-processor** (`WrkResultPostProcessor.java`) - Parses output, fetches metrics, generates reports
...
* **Script**: `health_live_check.lua`
* **Purpose**: Baseline performance without authentication
* **Expected** (150 connections, stress profile): 70,000+ req/s, P50 <2ms latency
* **Measured**: 88,700 req/s, P50 1.25ms (see link:../doc/Analysis-03.2026-Integration.adoc[Integration Analysis])
...
* **Measured**: 23,800 req/s, P50 5.49ms (see link:../doc/Analysis-03.2026-Integration.adoc[Integration Analysis])
...
|`wrk/wrk-health-live-results.txt`
|Raw WRK output for health endpoint

|`wrk/wrk-jwt-results.txt`
|Raw WRK output for JWT endpoint

|`gh-pages-ready/benchmark-data.json`
|Unified benchmark report (HTML reports generated alongside)

|`prometheus/healthCheck-metrics.json`
|Prometheus metrics captured during health benchmark

|`prometheus/jwtValidation-metrics.json`
|Prometheus metrics captured during JWT benchmark

|`quarkus.log`
|Application logs from benchmark run (written by Quarkus file logging)

|`keycloak-logs-<timestamp>.txt`
|Keycloak container logs dumped after benchmarks
|===
...
cat target/benchmark-results/wrk/wrk-health-live-results.txt
cat target/benchmark-results/wrk/wrk-jwt-results.txt

# Analyze unified benchmark report
jq '.benchmarks' target/benchmark-results/gh-pages-ready/benchmark-data.json

# Check Prometheus metrics
jq '.' target/benchmark-results/prometheus/healthCheck-metrics.json
jq '.' target/benchmark-results/prometheus/jwtValidation-metrics.json

# View application logs
cat target/benchmark-results/quarkus.log
----
...
<execution>
    ...
    <configuration>
        ...
        <arguments>
            <argument>${wrk.script.dir}/custom_benchmark.sh</argument>
        </arguments>
        <outputFile>${wrk.output.dir}/wrk-custom-results.txt</outputFile>
...
The shell script should use `docker compose run --rm wrk` to execute WRK inside the Docker network
(see `health_live_benchmark.sh` and `jwt_benchmark.sh` for reference implementations).
...
* link:../../token-sheriff-quarkus-parent/token-sheriff-quarkus-integration-tests/README.adoc[Integration Test Infrastructure]