#!/bin/bash
# Mock JWT benchmark runner wrapper for Maven execution
# Identical to jwt_benchmark.sh but targets /mock-jwt/validate endpoint
# Purpose: Measure CDI/serialization overhead without JWT library validation
...
BENCHMARK_NAME="mockJwtValidation"
...
# Run wrk with the mock JWT script using docker compose
...
docker compose run --rm -e TOKEN_DATA="$TOKEN_DATA" wrk \
    ...
    --timeout "$WRK_TIMEOUT" \
    --latency \
    -s /scripts/mock_jwt_benchmark.lua \
    "$SERVICE_URL/mock-jwt/validate"
