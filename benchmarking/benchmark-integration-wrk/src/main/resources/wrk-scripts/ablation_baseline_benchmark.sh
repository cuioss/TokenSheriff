#!/bin/bash
# Ablation Baseline benchmark runner wrapper for Maven execution
# Identical to mock_jwt_benchmark.sh but targets /mock-jwt/baseline endpoint
...
BENCHMARK_NAME="ablationBaseline"
...
# Run wrk with the ablation baseline script using docker compose
...
docker compose run --rm -e TOKEN_DATA="$TOKEN_DATA" wrk \
    ...
    --timeout "$WRK_TIMEOUT" \
    --latency \
    -s /scripts/ablation_baseline_benchmark.lua \
    "$SERVICE_URL/mock-jwt/baseline"
