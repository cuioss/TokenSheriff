#!/bin/bash
# Ablation Header-Only benchmark runner wrapper for Maven execution
# Identical to mock_jwt_benchmark.sh but targets /mock-jwt/header-only endpoint
# Purpose: Measure Vert.x header resolution overhead (no token parsing, no validation)
...
BENCHMARK_NAME="ablationHeaderOnly"

# Service URL uses Docker service name (configured in docker-compose.yml)
SERVICE_URL="https://token-sheriff-integration-tests:8443"
...
# Run wrk with the ablation header-only script using docker compose
...
docker compose run --rm -e TOKEN_DATA="$TOKEN_DATA" wrk \
    ...
    --timeout "$WRK_TIMEOUT" \
    --latency \
    -s /scripts/ablation_header_only_benchmark.lua \
    "$SERVICE_URL/mock-jwt/header-only"
