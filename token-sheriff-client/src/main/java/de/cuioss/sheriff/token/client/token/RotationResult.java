 ...
 * <p>
 * Carries the freshly validated access token, the refresh token to present on the next refresh, and
 * whether the authorization server rotated the refresh token. When {@link #rotated()} is
 * {@code true} the {@link #refreshToken()} is the AS-issued successor and the presented token has
 * been superseded; when {@code false} the AS chose not to rotate (RFC 6749 §6 permits omitting a new
 * refresh token) and {@link #refreshToken()} is the still-valid presented token.
 *
...
long accessTokenExpiresInSeconds, boolean rotated) {
