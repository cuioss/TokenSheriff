...
* link:src/main/java/de/cuioss/sheriff/oauth/quarkus/config/IssuerConfigResolver.java[IssuerConfigResolver] - Resolves issuer configurations from properties
* link:src/main/java/de/cuioss/sheriff/oauth/quarkus/config/ParserConfigResolver.java[ParserConfigResolver] - Handles parser configuration
* link:src/main/java/de/cuioss/sheriff/oauth/quarkus/config/AccessTokenCacheConfigResolver.java[AccessTokenCacheConfigResolver] - Manages cache configuration
* link:src/main/java/de/cuioss/sheriff/oauth/quarkus/config/RetryStrategyConfigResolver.java[RetryStrategyConfigResolver] - Resolves retry strategy configuration
* link:src/main/java/de/cuioss/sheriff/oauth/quarkus/config/AccessLogFilterConfigResolver.java[AccessLogFilterConfigResolver] - Resolves access log filter configuration
* link:src/main/java/de/cuioss/sheriff/oauth/quarkus/mapper/ClaimMapperRegistry.java[ClaimMapperRegistry] - CDI-based claim mapper discovery and registration

==== JwtPropertyKeys
Configuration property key constants:

* link:src/main/java/de/cuioss/sheriff/oauth/quarkus/config/JwtPropertyKeys.java[JwtPropertyKeys]
...
* link:src/main/java/de/cuioss/sheriff/oauth/quarkus/producer/TokenValidatorProducer.java[TokenValidatorProducer]
...
* link:src/main/java/de/cuioss/sheriff/oauth/quarkus/producer/BearerTokenProducer.java[BearerTokenProducer]
...
Quarkus recorder placeholder for future runtime initialization:

* link:src/main/java/de/cuioss/sheriff/oauth/quarkus/runtime/OAuthSheriffRecorder.java[OAuthSheriffRecorder]
* Currently empty — all components are managed by CDI
* Available for future runtime initialization needs called from the deployment processor

==== OAuthSheriffDevUIRuntimeService
Runtime service for DevUI integration:

* link:src/main/java/de/cuioss/sheriff/oauth/quarkus/runtime/OAuthSheriffDevUIRuntimeService.java[OAuthSheriffDevUIRuntimeService]
...
sheriff.oauth.issuers.my-issuer.issuer-identifier=https://auth.example.com
sheriff.oauth.issuers.my-issuer.enabled=true
sheriff.oauth.issuers.my-issuer.expected-audience=my-application

# JWKS configuration
sheriff.oauth.issuers.my-issuer.jwks.http.url=https://auth.example.com/jwks
sheriff.oauth.issuers.my-issuer.jwks.http.refresh-interval-seconds=3600
sheriff.oauth.issuers.my-issuer.jwks.http.connect-timeout-seconds=5
sheriff.oauth.issuers.my-issuer.jwks.http.read-timeout-seconds=10

# Algorithm preferences
sheriff.oauth.issuers.my-issuer.algorithm-preferences=RS256,RS384,RS512
----
...
* xref:../../doc/security/security-reference.adoc[Security Reference]
