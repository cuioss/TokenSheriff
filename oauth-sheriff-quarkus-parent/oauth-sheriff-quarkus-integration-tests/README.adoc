...
./mvnw clean verify -Pmulti-idp-tests -pl oauth-sheriff-quarkus-parent/oauth-sheriff-quarkus-integration-tests -am
...
ls -la oauth-sheriff-quarkus-parent/oauth-sheriff-quarkus-integration-tests/target/jfr-results/
----
...
Tests in `JwtValidationEndpointDexIT` are automatically skipped when Dex is not running.

See link:../../doc/specification/multi-idp-testing.adoc[Multi-IDP Testing Specification] for the full architecture and provider roadmap.
...
oauth-sheriff-quarkus-integration-tests/
├── src/main/
│   ├── java/de/cuioss/sheriff/oauth/integration/
...
├── src/test/java/de/cuioss/sheriff/oauth/integration/
...
* **HealthCheckResponsivenessIT**: Health endpoint responsiveness testing
...
* **JwtValidationEndpointDexIT**: Dex provider validation (multi-IDP, skipped when Dex not running)
* **TestRealm**: Test realm configuration helper (supports Keycloak and Dex)
...
# Metrics
curl -k https://localhost:10443/q/metrics | grep sheriff_oauth
----
