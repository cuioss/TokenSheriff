...
* **Core types**: `TokenContent extends JsonWebToken` — every validated token IS a `Principal` and `JsonWebToken`
* **Spec method signatures**: MP-JWT method names and return types are the primary API for standard claims
...
`TokenContent` extends `JsonWebToken`, making every validated token a `java.security.Principal`. This enables standard Jakarta Security integration:

[source,java]
----
@Inject JsonWebToken callerPrincipal;  // MP-JWT standard injection
@Inject Principal principal;            // Jakarta Security standard injection
----

The spec method signatures are adopted as the **primary API**:
...
|`getSubject()`
|`String` (nullable)
|Replaces `Optional<String>` (convenience: `getSubjectOption()`)

|`getExpirationTime()`
|`long` (epoch seconds)
|Replaces `OffsetDateTime` (convenience: `getExpirationDateTime()`)

|`getIssuedAtTime()`
|`long` (epoch seconds)
|Replaces `OffsetDateTime` (convenience: `getIssuedAtDateTime()`)
...
|`getClaimNames()`
|`Set<String>`
|All claim keys in token

|`getClaim(String)`
|`<T> T`
|Generic claim access with type conversion
|===
