...
Configurable clock skew tolerance per issuer for `exp` and `nbf` validation, accommodating clock drift in distributed systems.

Planned configuration: `sheriff.oauth.issuers.<name>.clock-skew-seconds` (default: `0`)

=== Token Age Verification (`mp.jwt.verify.token.age`)

Maximum token age validation based on `iat` claim. Rejects tokens where `now - iat > maxAge + clockSkew`, even if not yet expired.

Planned configuration: `sheriff.oauth.issuers.<name>.max-token-age-seconds` (default: disabled)
...
`@Produces JsonWebToken` and `@Produces Principal` for standard MP-JWT injection patterns.
