...
# Verify Zitadel
curl -k https://localhost:3443/.well-known/openid-configuration
...
|No
|Yes
|Keycloak: realm roles, Zitadel: project roles via Actions. Dex has no roles concept.

|Bearer with-groups
|Yes
|No
|Yes
|Keycloak: group mappers, Zitadel: user metadata via Actions. Dex has groups but lacks other bearer requirements.
...
|ROLES |Yes |Via project role grants + Actions flattening into `roles` claim
|GROUPS |Yes |Via user metadata + Actions injecting `groups` claim
