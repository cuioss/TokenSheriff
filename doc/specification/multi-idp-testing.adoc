...
|Yes
|Yes
|Dex access tokens are valid RS256-signed JWTs. The `scope` claim is absent, which is compliant per RFC 9068 Section 2.2 (scope is OPTIONAL).
...
|Yes
|No
|Requires roles and custom scopes — see <<dex-limitations>>
...
The `bearerAuthProviders()` method source requires `ROLES + GROUPS + CUSTOM_SCOPES`. Dex has `GROUPS` but cannot provide `ROLES` or `CUSTOM_SCOPES`. A second full-featured IDP (e.g. Zitadel) would be needed to exercise bearer auth across multiple providers.
