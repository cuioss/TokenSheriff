...
All imports move from `de.cuioss.sheriff.oauth.` to `de.cuioss.sheriff.token.` (the sub-paths — `core.domain.token`, `core.exception`, `core.security`, `validation.jwks.http`, `core.test.*`, etc. — are unchanged).
