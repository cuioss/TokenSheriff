...
== Plan 05 (Client module) follow-up — transport cluster package moves

xref:../05-client-module/README.adoc[Plan 05] completed the transport re-packaging that Plan 03
deferred: the OIDC discovery + JWKS *transport* plumbing moved from the `validation` packages into
`de.cuioss.sheriff.token.commons.transport` (still shipped in the same `token-sheriff-validation`
artifact — *no new dependency*). The JWKS *crypto* types (`HttpJwksLoader`, `JWKSKeyLoader`,
`KeyInfo`, `jwks.parser`/`jwks.key`) stayed in `validation`.

Public types a consumer may import that changed package (names only, no behavioural change):

[cols="3,3"]
|===
| From | To

| `de.cuioss.sheriff.token.validation.ParserConfig`
| `de.cuioss.sheriff.token.commons.transport.ParserConfig`

| `de.cuioss.sheriff.token.validation.jwks.http.HttpJwksLoaderConfig`
| `de.cuioss.sheriff.token.commons.transport.HttpJwksLoaderConfig`

| `de.cuioss.sheriff.token.validation.jwks.JwksType`
| `de.cuioss.sheriff.token.commons.transport.JwksType`

| `de.cuioss.sheriff.token.validation.util.LoaderStatus`
| `de.cuioss.sheriff.token.commons.transport.LoaderStatus`

| `de.cuioss.sheriff.token.validation.util.LoadingStatusProvider`
| `de.cuioss.sheriff.token.commons.transport.LoadingStatusProvider`

| `de.cuioss.sheriff.token.validation.wellknown.*`
(`HttpWellKnownResolver`, `WellKnownConfig`)
| `de.cuioss.sheriff.token.commons.transport.*`

| `de.cuioss.sheriff.token.validation.json.{Jwks,JwkKey,WellKnownResult}`
| `de.cuioss.sheriff.token.commons.transport.{Jwks,JwkKey,WellKnownResult}`
|===

`HttpJwksLoader` and the JWKS key types keep their `de.cuioss.sheriff.token.validation.jwks..`
packages. The `generators` classifier is unchanged.

Consumer fix (same style as Step 2):

[source,bash]
----
grep -rl 'de\.cuioss\.sheriff\.token\.validation\.\(ParserConfig\|jwks\.http\.HttpJwksLoaderConfig\|jwks\.JwksType\|util\.\(LoaderStatus\|LoadingStatusProvider\)\|wellknown\.\|json\.\(Jwks\|JwkKey\|WellKnownResult\)\)' \
     --include='*.java' . \
  | xargs sed -i '' \
      -e 's/validation\.ParserConfig/commons.transport.ParserConfig/g' \
      -e 's/validation\.jwks\.http\.HttpJwksLoaderConfig/commons.transport.HttpJwksLoaderConfig/g' \
      -e 's/validation\.jwks\.JwksType/commons.transport.JwksType/g' \
      -e 's/validation\.util\.LoaderStatus/commons.transport.LoaderStatus/g' \
      -e 's/validation\.util\.LoadingStatusProvider/commons.transport.LoadingStatusProvider/g' \
      -e 's/validation\.wellknown\./commons.transport./g' \
      -e 's/validation\.json\.Jwks/commons.transport.Jwks/g' \
      -e 's/validation\.json\.JwkKey/commons.transport.JwkKey/g' \
      -e 's/validation\.json\.WellKnownResult/commons.transport.WellKnownResult/g'
----

(These moves ship in the first release that includes Plan 05; earlier releases are unaffected.)
