...
token-sheriff-validation                  (one artifact)
  de.cuioss.sheriff.token.commons.        base layer — deps: cui-http only
    transport/  error/  events/  metrics/
        ▲   ArchUnit: commons must NOT depend on …validation.. / …client..
...
* `commons` is the package root `de.cuioss.sheriff.token.commons` (sub-packages `transport`/`error`/`events`/`metrics`), shipped in the `token-sheriff-validation` artifact, depending on `cui-http` only.
...
* `…commons..` may depend on: the JDK, `cui-http`, `cui-tooling` — and *not* on `…validation..` or `…client..`.
...
* [ ] `de.cuioss.sheriff.token.commons.*` packages exist in `token-sheriff-validation`, depend only on `cui-http`; *no new artifact*
* [ ] ArchUnit rules pass: `commons` never depends on `…validation..`/`…client..`; no cross-boundary cycles
