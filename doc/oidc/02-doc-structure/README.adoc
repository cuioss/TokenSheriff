...
Reshape the conceptual layer into peer capabilities — `validation`, `commons`, and `client` — one per *architectural boundary*, rather than extending the existing validation docs with the new content. (These map to *two* Maven modules: `validation` and `client`; `commons` is the ArchUnit-enforced base *layer* inside `validation` but keeps its own doc capability — see xref:../decision-oidc-module.adoc[the module-architecture decision].)
...
* It mirrors the *architectural* boundary (one doc capability per layer — the commons base, validation, client), keeping doc↔code traceability clean even where the base is a package layer rather than its own module.
* It is consistent with the modularity already present (folders, per-module docs) — an increment, not a new paradigm.

The already-modular parts stay as they are: module `doc/` dirs continue per-module, and `specification/` stays a folder (now per-capability). `LogMessages.adoc` stays a single *global* registry covering all capabilities — it is not split or duplicated per capability. The cross-cutting *error model, events and metrics* are *owned by the `commons` capability* (the base layer in validation), not a separate bucket; a small `shared/` area holds only a cross-capability glossary.
...
Module-level docs are unchanged in pattern: the new client module (`token-sheriff-client`) gets its own `doc/` for usage/config/dev. `commons` is a package layer inside `token-sheriff-validation`, so its usage/config docs live within validation's module `doc/` — only its *conceptual* capability docs (requirements/spec/threat-model) sit under `doc/commons/`. Log records stay in the single global `doc/LogMessages.adoc` — not per module or per capability.
