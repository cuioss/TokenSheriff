...
| A DPoP / mTLS-bound token is asserted to keep its binding across storage and refresh.
  "mTLS-bound" means an existing `cnf.x5t#S256` binding, not mTLS client authentication, which
  is alpha (xref:../../adr/0012-mTLS_client_authentication_is_an_alpha_capability_DPoP_is_the_sender-constraining_and_client-authentication_direction.adoc[ADR-0012]).
|===
...
The proof is a *matched negative control*: suppress every test execution that contributes coverage
to the target fully-qualified class name, run the lane the gate is supposed to run in, and confirm
the build fails naming the *expected fully-qualified class name*. Naming it is the load-bearing
part. A failure that names some other class proves a different rule fired; a failure that names no
class proves only that something broke. Without the name, a passing negative control is consistent
with the gate being vacuous.
...
gate closes. After editing the list, confirm the gate still fires by suppressing every test
execution that contributes coverage to the entry for one run — several test classes commonly share
one production class, and suppressing only one of them can leave it above threshold, which reports
the same green as a gate that does not fire.
====
