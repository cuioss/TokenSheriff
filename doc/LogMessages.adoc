...
|JWTValidation-001 |TOKEN |TokenValidator initialized with %s |Logged when TokenValidator is initialized with issuer configurations
...
|JWTValidation-011 |JWE |JWE decryption enabled with %s decryption key(s) |Logged when JWE decryption is configured on TokenValidator with the number of available decryption keys
|===
...
|JWTValidation-104 |TOKEN |Invalid JWT Token format: expected 3 parts (JWS) or 5 parts (JWE) but got %s |Logged when the JWT token format is invalid
...
|JWTValidation-149 |DPOP |DPoP proof is required but the DPoP HTTP header is missing |Logged when a DPoP proof is expected (cnf.jkt present or dpop.required=true) but the DPoP HTTP header is absent
|JWTValidation-150 |DPOP |DPoP proof has invalid format: %s |Logged when the DPoP proof JWT cannot be decoded or has structural issues (wrong typ, unsupported algorithm, missing jwk, invalid signature)
|JWTValidation-151 |DPOP |DPoP proof iat claim is outside acceptable freshness window |Logged when the DPoP proof iat claim is too old or too far in the future
|JWTValidation-152 |DPOP |DPoP proof JWK thumbprint '%s' does not match token cnf.jkt '%s' |Logged when the computed JWK Thumbprint (RFC 7638) of the DPoP proof's public key does not match the cnf.jkt claim in the access token
|JWTValidation-153 |DPOP |DPoP proof replay detected for jti: %s |Logged when a DPoP proof with a previously seen jti value is detected (replay attack)
|JWTValidation-154 |DPOP |DPoP proof is missing required claim: %s |Logged when the DPoP proof JWT body is missing a required claim (jti, iat, or ath)
|JWTValidation-155 |DPOP |DPoP proof ath claim does not match access token hash |Logged when the DPoP proof ath claim (SHA-256 hash of the access token) does not match the actual access token
|JWTValidation-156 |DPOP |DPoP is required but access token does not contain cnf.jkt claim |Logged when DPoP validation is required or a DPoP proof is provided but the access token lacks the cnf.jkt claim
|JWTValidation-157 |JWE |Failed to decrypt JWE token: %s |Logged when JWE decryption fails due to cryptographic errors (wrong key, tampered content, etc.)
|JWTValidation-158 |JWE |Unsupported JWE algorithm: alg=%s, enc=%s |Logged when a JWE token uses an unsupported or rejected algorithm combination
|JWTValidation-159 |JWE |Received JWE token but no decryption configuration is available |Logged when a 5-part JWE token is received but no JweDecryptionConfig was configured on TokenValidator
|JWTValidation-160 |JWE |No decryption key found for key ID: %s |Logged when the JWE header references a kid that has no matching decryption key and no default key is configured
|JWTValidation-161 |JWE |Unsupported JWE compression algorithm: %s |Logged when a JWE token uses an unsupported compression algorithm (only DEF/DEFLATE is supported)
|JWTValidation-162 |JWE |Nested JWE tokens are not allowed |Logged when decrypting a JWE token reveals another JWE (5-part) token instead of the expected inner JWS
|===
...
|OAUTH_SHERIFF_QUARKUS-001 |RESOLVING_ISSUER_CONFIGURATIONS |"Resolving issuer configurations from properties" |Indicates the start of issuer configuration resolution from application properties
|OAUTH_SHERIFF_QUARKUS-002 |RESOLVED_ISSUER_CONFIGURATION |"Resolved issuer configuration: %s" |Shows a single resolved issuer configuration with its details
|OAUTH_SHERIFF_QUARKUS-003 |RESOLVED_ENABLED_ISSUER_CONFIGURATIONS |"Resolved %s enabled issuer configurations" |Summary count of all enabled issuer configurations found
|OAUTH_SHERIFF_QUARKUS-004 |RESOLVED_PARSER_CONFIG |"Resolved ParserConfig: maxTokenSize=%s bytes, maxPayloadSize=%s bytes, maxStringLength=%s, maxBufferSize=%s" |Shows the resolved JWT parser configuration limits
|===
...
|OAUTH_SHERIFF_QUARKUS-011 |INITIALIZING_JWT_VALIDATION_COMPONENTS |"Initializing JWT validation components from configuration" |Indicates the start of JWT validation component initialization
|OAUTH_SHERIFF_QUARKUS-012 |JWT_VALIDATION_COMPONENTS_INITIALIZED |"JWT validation components initialized successfully with %s issuers" |Confirms successful initialization with issuer count
|OAUTH_SHERIFF_QUARKUS-013 |RESOLVING_ACCESS_LOG_FILTER_CONFIG |"Resolving access log filter configuration from properties" |Indicates the start of access log filter configuration resolution
|OAUTH_SHERIFF_QUARKUS-014 |CLAIM_MAPPER_REGISTRY_INITIALIZED |"Claim mapper registry initialized with %s custom mapper(s): %s" |Shows the number of custom claim mappers discovered and their names
|OAUTH_SHERIFF_QUARKUS-015 |NO_CUSTOM_CLAIM_MAPPERS_DISCOVERED |"No custom claim mappers discovered" |Indicates that no custom claim mappers were found during CDI discovery
|===
...
|OAUTH_SHERIFF_QUARKUS-021 |INITIALIZING_JWT_METRICS_COLLECTOR |"Initializing JwtMetricsCollector" |Indicates the start of JWT metrics collector initialization
|OAUTH_SHERIFF_QUARKUS-022 |JWT_METRICS_COLLECTOR_INITIALIZED |"JwtMetricsCollector initialized with %s event types" |Confirms successful initialization with event type count
|===
...
|OAUTH_SHERIFF_QUARKUS-031 |JWE_DECRYPTION_CONFIG_RESOLVING |"Resolving JWE decryption configuration from properties" |Indicates the start of JWE decryption configuration resolution
|OAUTH_SHERIFF_QUARKUS-032 |JWE_DECRYPTION_CONFIG_RESOLVED |"JWE decryption configuration resolved with %s key(s)" |Shows the number of JWE decryption keys resolved
|OAUTH_SHERIFF_QUARKUS-033 |JWE_CONFIG_CHECK |"JWE config check: singleKeyPath=%s, keystorePath=%s, multiKeys=%s" |Logs the JWE configuration sources being checked
|OAUTH_SHERIFF_QUARKUS-034 |JWE_DECRYPTION_NOT_CONFIGURED |"No JWE decryption configuration found - JWE support disabled" |Indicates that no JWE decryption configuration was found
|===
...
|OAUTH_SHERIFF_QUARKUS-041 |RESOLVING_ACCESS_TOKEN_CACHE_CONFIG |"Resolving access token cache configuration from properties" |Indicates the start of access token cache configuration resolution
|OAUTH_SHERIFF_QUARKUS-042 |ACCESS_TOKEN_CACHE_DISABLED |"Access token cache disabled (maxSize=0)" |Indicates that the access token cache is disabled
|OAUTH_SHERIFF_QUARKUS-043 |ACCESS_TOKEN_CACHE_CONFIGURED |"Access token cache configured: maxSize=%s, evictionIntervalSeconds=%s" |Shows the configured access token cache settings
|===
...
|OAUTH_SHERIFF_QUARKUS-051 |CLEARING_JWT_METRICS |"Clearing all JWT metrics" |Indicates that all JWT metrics are being cleared
|OAUTH_SHERIFF_QUARKUS-052 |JWT_METRICS_CLEARED |"JWT metrics cleared successfully" |Confirms successful clearing of JWT metrics
|===
...
|OAUTH_SHERIFF_QUARKUS-060 |JWKS_STARTUP_SERVICE_INITIALIZED |"JWKS startup service activated for background key loading" |Indicates that the JWKS startup service has been activated
|OAUTH_SHERIFF_QUARKUS-061 |STARTING_ASYNCHRONOUS_JWKS_INITIALIZATION |"Starting asynchronous JWKS initialization for %s issuer(s)" |Indicates the start of background JWKS loading with issuer count
|OAUTH_SHERIFF_QUARKUS-062 |NO_ISSUER_CONFIGURATIONS_FOUND |"No issuer configurations found - skipping JWKS initialization" |Indicates that JWKS initialization is skipped due to no configured issuers
|OAUTH_SHERIFF_QUARKUS-063 |BACKGROUND_JWKS_INITIALIZATION_COMPLETED |"Background JWKS initialization completed successfully" |Confirms successful completion of background JWKS initialization
|OAUTH_SHERIFF_QUARKUS-064 |BACKGROUND_JWKS_LOADING_COMPLETED_FOR_ISSUER |"Background JWKS loading completed for issuer: %s with status: %s" |Shows the completion status for a specific issuer's JWKS loading
|===
...
|OAUTH_SHERIFF_QUARKUS-065 |CUSTOM_ACCESS_LOG_FILTER_INITIALIZED |"CustomAccessLogFilter initialized: %s" |Shows the initialized access log filter configuration
|OAUTH_SHERIFF_QUARKUS-066 |ACCESS_LOG_ENTRY |"%s" |Individual access log entry in the configured format
|===
...
|OAUTH_SHERIFF_QUARKUS-100 |ERROR_CHECKING_JWKS_LOADER |"Error checking JWKS loader for issuer %s: %s" |Indicates an error occurred while checking JWKS loader status in health check
|===
...
|OAUTH_SHERIFF_QUARKUS-128 |BEARER_TOKEN_REQUIREMENTS_NOT_MET_DETAILED |"Bearer token does not meet requirements. Missing scopes: %s, Missing roles: %s, Missing groups: %s" |Details about which requirements are missing from a bearer token
|===
...
|OAUTH_SHERIFF_QUARKUS-131 |BACKGROUND_JWKS_LOADING_FAILED_FOR_ISSUER |"Background JWKS loading failed for issuer %s: %s" |Indicates that background JWKS loading failed for a specific issuer
|OAUTH_SHERIFF_QUARKUS-133 |NO_MICROMETER_COUNTER_FOUND |"No Micrometer counter found for event type %s, delta %s lost" |Indicates that a metrics counter was not found and a delta value was lost
|OAUTH_SHERIFF_QUARKUS-134 |JWKS_LOADING_RETRY_WARNING |"JWKS loading failed for issuer %s: %s - will retry via background refresh" |Indicates that JWKS loading failed but will be retried
|OAUTH_SHERIFF_QUARKUS-135 |BACKGROUND_JWKS_ISSUES_WARNING |"Background JWKS initialization encountered issues: %s - on-demand loading will handle this" |Indicates issues during background JWKS initialization with fallback to on-demand loading
|OAUTH_SHERIFF_QUARKUS-136 |BEARER_TOKEN_ANNOTATION_NOT_FOUND |"@BearerAuth annotation not found on method or class: %s" |Indicates that the @BearerAuth annotation was not found on the intercepted method or its declaring class
|===
...
|OAUTH_SHERIFF_QUARKUS-141 |JWE_KEYSTORE_MISSING_ALIAS |"JWE keystore configured at %s but no key-alias specified — skipping keystore key loading" |Indicates that a JWE keystore was configured but no key alias was specified
|OAUTH_SHERIFF_QUARKUS-142 |JWE_KEY_LOADING_FAILED |"Failed to load JWE decryption keys, JWE support disabled: %s" |Indicates that JWE decryption key loading failed and JWE support is disabled
|===
...
|OAUTH_SHERIFF_QUARKUS-201 |VERTX_REQUEST_CONTEXT_UNAVAILABLE |"Vertx HttpServerRequest context is unavailable - no active request context found" |Indicates that the Vertx HTTP server request context could not be resolved
|===